...
from transformers import CvtForImageClassification, ConvNextForImageClassification, AutoImageProcessor
...
class Model(nn.Module):
    def __init__(self, model='densenet', eval=True, batch_size=32, num_features=128,
        ...
        if model == 'densenet':
            ...
            self.conv_net = models.densenet121(weights='DenseNet121_Weights.DEFAULT').to(device=device)
        elif model == 'resnet':
            self.forward_function = self.forward_conv
            self.conv_net = models.resnet50(weights='ResNet50_Weights.DEFAULT').to(device=device)
        elif model == 'vision':
            self.forward_function = self.forward_conv
            self.conv_net = models.vit_b_16(weights = 'ViT_B_16_Weights.DEFAULT').to(device=device)
        elif model == "swim":
            self.forward_function = self.forward_conv
            self.conv_net = models.swin_v2_b(weights='Swin_V2_B_Weights.DEFAULT').to(device=device)
            print(self.conv_net)
        elif model == "vgg":
            self.forward_function = self.forward_conv
            self.conv_net = models.vgg19(weights="VGG19_Weights.DEFAULT").to(device=device)
        elif model == "inception":
            self.forward_function = self.forward_conv
            self.conv_net = models.inception_v3(weights="Inception_V3_Weights.DEFAULT").to(device=device)
        elif model == "effNet":
            self.forward_function = self.forward_conv
            self.conv_net = EfficientNet.from_pretrained('efficientnet-b0').to(device=device)
        elif model == "KNet":
        ...
        """elif model == "dgmil":
            self.forward_function = self.forward_conv
            self.conv_net = Linear_projection_MAE().to(device=device)"""
        ...
        if use_dr and model != 'transformer':
            out_features = 4096 # From deep ranking, in features are from the pretrained models, note that given the model it is
            # retrieved somewhere else (classifier densenet, .fc resnet, ._fc effnet,..). Localisation can be found by displaying the model 
            # (print(self.conv_net)) 
            if model == 'densenet':
                self.conv_net.classifier = nn.Linear(self.conv_net.classifier.in_features, out_features).to(device=device)
            elif model == 'resnet':
                self.conv_net.fc = nn.Linear(self.conv_net.fc.in_features, out_features).to(device=device)
            elif model == 'vision' or model == 'swim' or model == 'vgg' or model == "inception" or model == "effNet" or model == "KNet":
                self.conv_net.fc = nn.Linear(1024, out_features).to(device=device)
            ...
            if model == 'densenet':
                self.conv_net.classifier = nn.Linear(self.conv_net.classifier.in_features, num_features).to(device=device)
            elif model == 'resnet':
                self.conv_net.fc = nn.Linear(self.conv_net.fc.in_features, num_features).to(device=device)
            elif model == "vgg" or model == "inception":
            	self.conv_net.fc = nn.Linear(1024, num_features).to(device=device)
            elif model == "effNet":
                self.conv_net._fc = nn.Linear(self.conv_net._fc.in_features, num_features).to(device=device)
            elif model == 'vision':
                self.conv_net.heads.head = nn.Linear(self.conv_net.heads.head.in_features, num_features).to(device=device)
            elif model == 'KNet':
                self.conv_net.module.fc_4 = nn.Linear(self.conv_net.module.fc_4.in_features, num_features).to(device=device)
            elif model == 'swim':
                self.conv_net.head = nn.Linear(self.conv_net.head.in_features, num_features).to(device=device)
            elif model == 'transformer':
